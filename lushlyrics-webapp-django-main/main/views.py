...
'''def signup(request):
   if request.method == 'POST':
      ...
      if User.objects.filter(username = username).exists():
         message = messages.warning(request, 'username already exists')
         context = {'message' : message}
         return render(request, 'signup.html', context)
      if User.objects.filter(email = email).exists():
         message = messages.warning(request, "email is already exists please try login")
         context = {"message" : message}
         return render(request, 'signup.html', context)
      if pass1 != pass2:
         message = messages.error(request, "password does not match..please try again")
         context = {'message': message}
         return render(request, 'signup.html', context)
...
def signup(request):
    messages.success(request, 'signup.html',{"message":"Welcome to cantact us..happy to hear from you."})
    if request.method == 'POST':
        username = request.POST['username']
        email = request.POST['email']
        password = request.POST['password']
        ...
        return render(request, 'signup.html', {'fuckoff': "hello world"})
